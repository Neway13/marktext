  ...
  <div class="tree-view">
    ...
    <div class="project-tree" v-if="projectTree">
      ...
      <div class="tree-wrapper" v-show="showDirectories">
        <folder v-for="(folder, index) of projectTree.folders" :key="index + 'folder'" :folder="folder" :depth="depth">
        </folder>
        ...
        <file v-for="(file, index) of projectTree.files" :key="index + 'file'" :file="file" :depth="depth"></file>
        <div class="empty-project" v-if="
          projectTree.files.length === 0 && projectTree.folders.length === 0
        ">
    ...
    <div class="my-project" @mouseenter.stop="toggleMyProject()" @mouseleave.stop="toggleMyProject()">
      <div class="title">
        <svg class="icon icon-arrow" :class="{ fold: !showMyProject }" aria-hidden="true" >
          <use xlink:href="#icon-arrow"></use>
        </svg>
        <span class="default-cursor text-overflow" >My Project</span>
      </div>
      <div class="my-project-list" v-show="showMyProject">
        <transition-group name="list">
          <div class="myProject" v-for="project of myProjects" :key="project.name" :project="project" @click="openProject(project.path)">
...
export default {
  ...
  methods: {
    ...
    openProject (path) {
      this.$store.dispatch('ASK_FOR_OPEN_PROJECT_PATH', path)
    }
  }
}
</script>
